                ...
                <div class="uk-width-large-1-4 uk-hidden-small">
                    <div class="aside">
                        ...
                        <div class="aside-category aside-product mt20">
                            ...
                            <div class="aside-body">
                                @foreach($widgets['products-hl']->object as $productCatalogue)
                                @php
                                // Lấy thông tin sản phẩm từ productCatalogue
                                $product = $productCatalogue->products->first(); // Lấy sản phẩm đầu tiên trong collection
                                $name = $product->name;
                                $canonical = write_url($product->languages->first()->pivot->canonical); // Canonical URL
                                $image = $product->image;
                                $price = getPrice($product); // Giả sử bạn đã có hàm getPrice để lấy giá sản phẩm
                                @endphp
                                <div class="aside-product uk-clearfix">
                                    ...
                                    <div class="info">
                                        ...
                                        {!! $price['html'] !!} <!-- Hiển thị giá sản phẩm -->
                                    </div>
                                </div>
                                @endforeach
                            </div>
